...
import {createNurse} from "../../services/nurses";
import {navigate} from "wouter/use-location";

function CreateEnfermero() {
  ...
  const handleSubmitForm = (e) => {
    ...
    if (nombres[1]) {
      newNurse.segundo_nombre = nombres[1];
    }

    createNurse(newNurse).then(() => {
  ...
  return (
    <form className="cita-form">
      <div className="cita-info">
        ...
        <label htmlFor="especialidad" className="input-label">
          especialidad
        </label>
        <input type="text" className="input-text" id="especialidad" ref={especialidadRef} />
        <label htmlFor="titulo" className="input-label">
          titulo
        </label>
        <input type="text" className="input-text" id="titulo" ref={tituloRef} />
      </div>
...
export default CreateEnfermero;
